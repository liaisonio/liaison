...
状态：标准构建已内置 Go 驱动；真实 DM8 经连接器的登录、基础 SQL 和元数据联调通过。
...
- 凭证遵循驱动原生解析规则，保留密码中的特殊字符；当前驱动无法正确表示
  含冒号的用户名，Liaison 会在拨号前拒绝此类用户名。
...
已验证：原生驱动调用连接器拨号钩子、特殊字符凭证解析、32 路并发隔离、
撤销取消、失败清理、跨用户凭证隔离、能力接口认证、参数拒绝及语句审计。
UI fixture 覆盖启用/禁用、中英文、明暗主题和桌面/手机。

真实 DM8 已通过已部署连接器验证：含特殊字符密码登录、建表、增删改查、
中文/NULL/日期/小数、Schema/表/列及对象详情、失败 SQL 的安全错误响应。
尚未验证：LOB 等特殊类型、真实长查询取消及数据库审计。
这些验证不代表所有达梦版本和功能均完整兼容。
