...
const pageSize = 10;
const auditProtocols = ['ssh', 'webssh', 'mysql', 'postgresql', 'redis', 'mongodb'] as const;
...
const AuditPage: React.FC = () => {
  ...
  const routeProtocol = routeSearch.get('protocol') || '';
  ...
  const protocolLabel = (protocol: AuditProtocol) => accessTypeLabel(protocol);
  ...
  }, [applied.protocol, isSSHProtocol, tr]);

  const selectProtocol = (protocol: AuditProtocol) => {
    setFilters((value) => ({ ...value, protocol, action: '' }));
    setApplied((value) => ({ ...value, protocol, action: '' }));
    setPage(1);
  };
  const actionOptions = isSSHProtocol ? ['execute', 'open_session', 'close_session'] : ['execute', 'open_session', 'close_session', 'test_connection', 'save_credential', 'delete_credential'];
  ...
  return <div className="liaison-page-stack audit-native-page">
    ...
    <div className="liaison-filter-bar audit-native-filter">
      <DateRangeField className="audit-native-time" label={tr('操作时间', 'Time')} start={filters.start} end={filters.end} startPlaceholder={tr('开始日期', 'Start date')} endPlaceholder={tr('结束日期', 'End date')} onStartChange={(start) => setFilters((value) => ({ ...value, start }))} onEndChange={(end) => setFilters((value) => ({ ...value, end }))} />
      <label className="liaison-compound"><span>{tr('关键词', 'Keyword')}</span><input value={filters.keyword} onChange={(event) => setFilters((value) => ({ ...value, keyword: event.target.value }))} placeholder={isSSHProtocol ? tr('命令或哈希', 'Command or hash') : tr('语句或哈希', 'Statement or hash')} /></label>
      ...
      <label className="liaison-compound"><span>{tr('操作', 'Operation')}</span><select value={filters.action} onChange={(event) => setFilters((value) => ({ ...value, action: event.target.value }))}><option value="">{tr('全部', 'All')}</option>{actionOptions.map((action) => <option key={action} value={action}>{actionLabel(action)}</option>)}</select></label>
      ...
      <div className="liaison-filter-actions"><Button onClick={() => { const reset = { ...emptyFilters, protocol: applied.protocol }; setFilters(reset); setApplied(reset); setPage(1); }}>{tr('重置', 'Reset')}</Button><Button variant="primary" onClick={() => { setApplied(filters); setPage(1); }}>{tr('查询', 'Search')}</Button></div>
    </div>
