...
import { Plus, Star, Trash2, ChevronDown, Cpu } from 'lucide-react';
...
type Configuration = { enabled: boolean; default_provider: string; providers: Provider[] };
const presets = [
...
export default function Models() {
  ...
  const submit = async (testProvider?: string) => {
    ...
    try {
      const response = await request<API.Response<Configuration>>(`/api/v1/settings/model${testProvider ? '/test' : ''}`, {
        ...
        data: { enabled: config.enabled, default_provider: config.default_provider, test_provider: testProvider,
  ...
  return <section className="settings-section settings-models">
    ...
    {config && <fieldset disabled={!!busy || !canWrite} className="model-provider-form">
      <div className="model-provider-toolbar"><label className="model-enable"><input type="checkbox" checked={config.enabled} onChange={e => setConfig({ ...config, enabled: e.target.checked })} />{tr('启用 Agent', 'Enable Agent')}</label><Button variant="primary" disabled={!!busy} onClick={() => void submit()}>{busy === 'save' ? tr('保存中…', 'Saving…') : tr('保存配置', 'Save settings')}</Button></div>
      <div className="model-provider-list">{config.providers.map(p => <article className="model-provider-card" key={p.id}>
        <div className="model-provider-header">
          <button className="model-provider-title" onClick={() => setExpanded(expanded === p.id ? '' : p.id)}><Cpu size={19} /><span>{presets.find(meta => meta.id === p.type)?.label || p.type}<small>{p.model || tr('尚未选择模型', 'No model selected')}</small></span><ChevronDown size={16} /></button>
      ...
      <div className="model-add-provider"><select aria-label={tr('选择模型提供方', 'Select model provider')} value={addType} onChange={e => setAddType(e.target.value)}>{presets.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select><Button onClick={addProvider}><Plus size={15} />{tr('添加提供方', 'Add provider')}</Button></div>
      <p className="model-save-hint">{tr('更改默认项或移除提供方后，点击保存配置生效。', 'Save settings to apply default changes or provider removals.')}</p>
    </fieldset>}
  </section>;
}
