...
function OrganizationFormModal({ open, organization, defaultParentId, organizations, onClose, onSubmit }: { open: boolean; organization?: API.Organization; defaultParentId?: number; organizations: API.Organization[]; onClose: () => void; onSubmit: (values: { name: string; description: string; parent_id?: number }) => Promise<void> }) {
  ...
  return <Modal open={open} width={520} title={organization ? tr('编辑组织', 'Edit organization') : tr('新建组织', 'New organization')} onClose={onClose} footer={<><Button onClick={onClose}>{tr('取消', 'Cancel')}</Button><Button variant="primary" disabled={!name.trim()} onClick={() => onSubmit({ name: name.trim(), description: description.trim(), parent_id: parentId ? Number(parentId) : undefined })}>{tr('确定', 'Save')}</Button></>}><div className="native-modal-form"><Field label={tr('组织名称', 'Name')} required><Input value={name} maxLength={128} onChange={(event) => setName(event.target.value)} /></Field><Field label={tr('上级组织', 'Parent organization')} hint={tr('默认挂在根组织下', 'Defaults to the root organization')}><Select value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">{tr('默认组织', 'Default organization')}</option>{flattenOrganizations(organizations).filter(({ organization: item }) => item.id !== organization?.id).map(({ organization: item, depth }) => <option key={item.id} value={item.id}>{'— '.repeat(depth)}{item.name}</option>)}</Select></Field><Field label={tr('描述', 'Description')}><Input value={description} maxLength={512} onChange={(event) => setDescription(event.target.value)} /></Field></div></Modal>;
}
