...
export default forwardRef<LLMConnectionHandle,{applicationId:string;accessId?:number;disabled:boolean;applicationOnly?:boolean;reuseUpstream?:boolean;onReady?:(ready:boolean)=>void}>(function LLMConnection({applicationId,accessId,disabled,applicationOnly=false,reuseUpstream=false,onReady},ref){
  const {tr}=useI18n();
  ...
  const defaultModelsSelected=useRef(false);
  const ready=loaded&&!!upstream&&!failed&&!probing&&pairs.length>0&&pairs.every(([a,b])=>a.trim()&&b.trim())&&new Set(pairs.map(([a])=>a.trim())).size===pairs.length;
  useEffect(()=>{onReady?.(ready);return()=>onReady?.(false);},[ready,onReady]);
  useEffect(()=>()=>probeRequest.current?.abort(),[]);
  useEffect(()=>{
    let alive=true;defaultModelsSelected.current=false;probeRequest.current?.abort();setProbing(false);setModels(undefined);setProbeError('');setLoaded(false);setFailed(false);setUpstream(undefined);setDirty(false);
    (async()=>{
  ...
  useEffect(()=>{if(loaded&&reuseUpstream)void fetchModels();},[loaded,reuseUpstream]);
  function validate(){
  ...
  async function fetchModels(){
    ...
    try{
      ...
      if(result.data.state==='auth_required')setProbeError(tr('上游认证失败，请检查上游 API 密钥。','Upstream authentication failed. Check the upstream API key.'));
        ...
        const discovered=[...new Set(result.data.models||[])];setModels(discovered);
        if(reuseUpstream&&!accessId&&!defaultModelsSelected.current){
          defaultModelsSelected.current=true;
          setPairs(old=>old.some(([a,m])=>a||m)?old:discovered.length?discovered.slice(0,100).map(m=>[m,m] as [string,string]):[['','']]);
        }
      }
  ...
  return <fieldset className="is-full liaison-initial-connection liaison-llm-connection" disabled={disabled||probing}>
    <legend>{reuseUpstream?tr('开放模型','Exposed models'):tr('模型连接','Model connection')}</legend>
    {reuseUpstream?<p>{tr('上游协议','Upstream protocol')} · {llmLabel(upstream.protocol)}</p>:<p>{applicationOnly?tr('获取列表会保存上游配置，不会创建访问或自动开放模型。','Fetching saves upstream settings. It does not create access or expose models.'):tr('上游配置属于应用，修改会影响该应用的其他访问；模型映射仅属于本访问。','Upstream settings are shared by this application’s accesses. Model mappings apply only to this access.')}</p>}
    <div className="liaison-initial-connection-fields">
      ...
      {!reuseUpstream&&<div className="is-full"><Button loading={probing} onClick={()=>void fetchModels()}>{tr('保存上游并获取模型列表','Save upstream & fetch models')}</Button></div>}
      {reuseUpstream&&probing&&<p className="is-full" role="status">{tr('正在获取模型…','Fetching models…')}</p>}
      ...
      {models!==undefined&&<div className="is-full liaison-llm-models" role="group" aria-label={tr('发现的模型','Discovered models')}>
        {!models.length?<p>{tr('上游返回空列表。可检查服务后重试，或在访问中手动填写模型。','The upstream returned no models. Check the service and retry, or enter models manually when configuring access.')}</p>:<>
          <p>{applicationOnly?tr('已发现的模型；在创建访问时选择要开放的模型。','Discovered models. Choose which to expose when creating access.'):reuseUpstream&&models.length===1?tr('本访问开放以下模型。','This access exposes the model below.'):tr('选择本访问开放的模型，不修改应用配置。','Choose models to expose through this access. Application settings stay unchanged.')}</p>
          {models.map(model=>applicationOnly||(reuseUpstream&&models.length===1&&pairs.some(([,m])=>m===model))?<div key={model}><code>{model}</code></div>:<label className="liaison-checkbox" key={model}><input type="checkbox" checked={pairs.some(([,m])=>m===model)} disabled={!pairs.some(([,m])=>m===model)&&pairs.filter(([a,m])=>a||m).length>=100} onChange={e=>setPairs(old=>e.target.checked?[...old.filter(([a,m])=>a||m),[model,model]]:old.filter(([,m])=>m!==model))}/><code>{model}</code></label>)}
        </>}
      </div>}
      {!applicationOnly&&<>
      ...
      <details className="is-full liaison-llm-mappings" open={reuseUpstream?(!!probeError||models?.length===0||undefined):!models?.length}>
      <summary>{reuseUpstream?tr('高级设置','Advanced settings'):tr('对外名称与手动映射','Public names & manual mappings')}</summary>
      {reuseUpstream&&<Button loading={probing} onClick={()=>void fetchModels()}>{tr('刷新模型列表','Refresh models')}</Button>}
