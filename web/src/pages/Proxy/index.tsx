...
const ProxyPage: React.FC = () => {
  ...
  const columns: Column<API.Proxy>[] = [
    ...
    ...endpointColumn,
    { key: 'status', title: tr('状态', 'Status'), width: 90, render: (row) => <StatusPill tone={row.effective_status === 'active' || row.status === 'running' ? 'success' : 'neutral'}>{row.effective_status_message || row.status}</StatusPill> },
