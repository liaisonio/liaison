...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('.webssh-completion button > span').textContent(),'ip a');
  assert.equal(await page.locator('.typed').evaluate(e=>getComputedStyle(e).maxWidth),'none');
  assert.equal(await page.locator('.typed').evaluate(e=>getComputedStyle(e).textOverflow),'clip');
  await page.screenshot({path:'/tmp/liaison-completion-ip.png'});
