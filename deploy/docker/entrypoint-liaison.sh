...
: "${AGENT_OUTPUT_LANGUAGE:=zh}"
case "$AGENT_OUTPUT_LANGUAGE" in zh|en) ;; *) echo 'AGENT_OUTPUT_LANGUAGE must be zh or en' >&2; exit 1 ;; esac
export AGENT_OUTPUT_LANGUAGE
...
if [ ! -f "$CONF_DIR/liaison.yaml" ]; then
    ...
    envsubst '${AGENT_OUTPUT_LANGUAGE} ${FRONTIER_PORT} ${FRONTIER_CONTROLPLANE_PORT} ${MANAGER_PORT} ${SERVER_URL} ${JWT_SECRET} ${GUACD_ADDR} ${GUACD_BRIDGE_ADDR} ${GUACD_BRIDGE_HOST} ${AGENT_ENABLED} ${AGENT_BASE_URL} ${AGENT_MODEL} ${AGENT_REQUEST_TIMEOUT} ${AGENT_MAX_MODEL_STEPS} ${AGENT_APPROVAL_EXPIRY} ${WEB_TLS_CERTS} ${LIAISON_WEB_DOMAIN}' \
